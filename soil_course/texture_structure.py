...
def usda_texture_class(sand, clay):
    """Return the USDA textural class for sand and clay percentages.
...
def check_usda_texture_examples():
    ...
    for sand, clay, expected in examples:
        print(
            f"sand={sand:3d}%  clay={clay:3d}%  ->  "
            f"{usda_texture_class(sand, clay):16s}  (expected ~ {expected})"
        )
...
def plot_texture_triangle(sand=40.0, clay=20.0, ax=None):
    """Draw a simplified USDA texture triangle and mark one soil point."""
    silt = 100.0 - sand - clay
...
def interactive_texture_triangle():
    """Interactive sliders for sand and clay percentages."""
    if not HAS_WIDGETS:
        print("ipywidgets not installed - call plot_texture_triangle(sand=.., clay=..) instead.")
        return plot_texture_triangle(sand=60, clay=10)

    sand_sl = widgets.FloatSlider(value=40, min=0, max=100, step=1, description="sand %", continuous_update=False)
    clay_sl = widgets.FloatSlider(value=20, min=0, max=100, step=1, description="clay %", continuous_update=False)

    def _update(sand, clay):
        if sand + clay > 100:
            print(f"sand + clay = {sand + clay:.0f}% > 100% -> impossible soil. Reduce one slider.")
            return
        plot_texture_triangle(sand=sand, clay=clay)

    display(widgets.interactive(_update, sand=sand_sl, clay=clay_sl))


def percent_finer(d, d50=0.05, sigma=1.2):
    """Percentage of soil mass finer than diameter d (mm), lognormal model."""
    d = np.asarray(d, dtype=float)
    return 100.0 * 0.5 * (1.0 + erf((np.log(d) - np.log(d50)) / (sigma * np.sqrt(2.0))))


def diameter_at_percent(p, d50=0.05, sigma=1.2):
    """Diameter at which the percentage finer curve reaches p percent."""
    return d50 * np.exp(sigma * np.sqrt(2.0) * erfinv(2.0 * p / 100.0 - 1.0))


def classify_grain_curve(d50=0.05, sigma=1.2):
    """Classify a lognormal grain-size curve by fractions and grading."""
    ...
    fractions = {"clay": float(clay), "silt": float(silt), "sand": float(sand), "gravel": float(gravel)}
    dominant = max(fractions, key=fractions.get)

    D10 = float(diameter_at_percent(10, d50, sigma))
    D30 = float(diameter_at_percent(30, d50, sigma))
    D60 = float(diameter_at_percent(60, d50, sigma))
    Cu = D60 / D10
    Cc = D30**2 / (D10 * D60)
...
def plot_grain_curve(d50=0.05, sigma=1.2, ax=None):
    """Plot a lognormal percentage-finer grain-size curve."""
    own_axes = ax is None
    if own_axes:
    ...
    ax.set_title(
        ...
        f"classification: {info['name'].upper()}",
        fontsize=11,
    )
...
def interactive_grain_curve():
    """Interactive sliders for d50 and sigma.

    Wrapped with a lambda so ipywidgets does not try to create a widget for ax=None.
    """
    if not HAS_WIDGETS:
        print("ipywidgets not installed - call plot_grain_curve(d50=.., sigma=..) instead.")
        return plot_grain_curve(d50=0.3, sigma=0.6)

    logd50_sl = widgets.FloatSlider(value=np.log10(0.3), min=-3.5, max=1.3, step=0.05, description="log10 d50", continuous_update=False)
    sigma_sl = widgets.FloatSlider(value=0.6, min=0.15, max=2.5, step=0.05, description="sigma", continuous_update=False)
    display(widgets.interactive(lambda logd50, sigma: plot_grain_curve(d50=10.0**logd50, sigma=sigma), logd50=logd50_sl, sigma=sigma_sl))


def curve_to_triangle(d50=0.05, sigma=1.2):
    """Extract sand/silt/clay from a grain-size curve and place it on the triangle."""
    info = classify_grain_curve(d50=d50, sigma=sigma)
    f = info["fractions"]
    ...
    if f["gravel"] > 1:
        axes[1].text(
            0.5,
            -0.02,
            f"({f['gravel']:.0f}% gravel removed before renormalising)",
...
def interactive_curve_to_triangle():
    """Interactive version of curve_to_triangle."""
    if not HAS_WIDGETS:
        print("ipywidgets not installed - call curve_to_triangle(d50=.., sigma=..) instead.")
        return curve_to_triangle(d50=0.08, sigma=1.6)

    logd50_sl = widgets.FloatSlider(value=np.log10(0.08), min=-3.5, max=1.3, step=0.05, description="log10 d50", continuous_update=False)
    sigma_sl = widgets.FloatSlider(value=1.6, min=0.15, max=2.5, step=0.05, description="sigma", continuous_update=False)
    display(widgets.interactive(lambda logd50, sigma: curve_to_triangle(d50=10.0**logd50, sigma=sigma), logd50=logd50_sl, sigma=sigma_sl))


PACKINGS = {
    "dense (rhombohedral)": 0.26,
    "random close": 0.36,
    "loose (cubic)": 0.48,
    "card-house clay": 0.70,
}


def bulk_density(porosity, particle_density=2.65):
    """Dry bulk density (g/cm3) from porosity and particle density."""
...
def bimodal_pore_distribution(r_textural=0.5, r_structural=50.0, sigma=0.5, f_struct=0.5, n_classes=400):
    """A bimodal pore-size distribution with textural and structural modes."""
    if r_textural <= 0 or r_structural <= 0 or sigma <= 0:
        raise ValueError("r_textural, r_structural, and sigma must be positive.")
    if not 0 <= f_struct <= 1:
        raise ValueError("f_struct must be between 0 and 1.")
...
def plot_bimodal_pores(r_textural=0.5, r_structural=50.0, sigma=0.5, f_struct=0.5):
    """Plot the bimodal/dual-porosity pore-size distribution."""
    r, vf = bimodal_pore_distribution(r_textural=r_textural, r_structural=r_structural, sigma=sigma, f_struct=f_struct)
...
def interactive_bimodal_pores():
    """Interactive sliders for bimodal pore distribution."""
    if not HAS_WIDGETS:
    ...
    def _show(f_struct, r_textural, r_structural, sigma):
        if r_textural >= r_structural:
            ...
            return
        plot_bimodal_pores(r_textural=r_textural, r_structural=r_structural, sigma=sigma, f_struct=f_struct)
...
def drainage_split(r_textural=0.5, r_structural=50.0, sigma=0.5, f_struct=0.5, cutoff_um=25.0):
    """Return freely-draining and retentive pore-volume fractions."""
    r, vf = bimodal_pore_distribution(r_textural=r_textural, r_structural=r_structural, sigma=sigma, f_struct=f_struct)
    draining = float(vf[r >= cutoff_um].sum())
    retentive = float(vf[r < cutoff_um].sum())
...
def print_drainage_table(cutoff_um=25.0):
    ...
    for fs in [0.0, 0.25, 0.5, 0.75, 1.0]:
        draining, retentive = drainage_split(f_struct=fs, cutoff_um=cutoff_um)
        print(f"{fs:12.2f}{draining:12.2f}{retentive:12.2f}")
